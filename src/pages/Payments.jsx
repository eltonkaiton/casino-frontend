...
function Payments() {
  ...
  const fetchPayments = async () => {
    try {
      ...
      const bookingPaymentsData = bookings
        .filter(booking => booking.paymentStatus === "Paid" || booking.paymentStatus === "Approved")
        .map(booking => ({
          id: booking._id,
          source: "booking",
      ...
      try {
        ...
        ordersPaymentsData = orders
          .filter(order => order.status === "Delivered" || order.status === "Paid" || order.paymentStatus === "Paid")
          .map(order => ({
            id: order._id,
            source: "order",
      ...
      const completedPayments = allPaymentsData.filter(p => p.status === "Paid" || p.status === "Approved").length;
  ...
  const generateReportData = () => {
    ...
    const reportStats = {
      ...
      paid: filteredData.filter(p => p.status === "Paid" || p.status === "Approved").length,
      totalAmount: totalAmount,
    };
  ...
  const generateReportHTML = () => {
    ...
    const reportTitle = reportType === "all" ? "All Payments Report" : 
                        reportType === "bookings" ? "Booking Payments Report" : "Order Payments Report";
      ...
      <body>
        <div class="report-container">
          ...
          <div class="report-footer">
            ...
            <p>Casino Events - Official Payment Report</p>
          </div>
        </div>
      </body>
      </html>
  ...
  const StatCard = ({ title, value, color, bgColor, icon }) => (
    <div className="col-md-3 mb-3">
      <div className="card h-100 border-0 shadow-sm" style={{ backgroundColor: bgColor }}>
  ...
  return (
    <div className="container-fluid px-4 mt-4">
      ...
      <div className="row mb-4">
        <StatCard title="Total Payments" value={stats.totalPayments} color="#4F46E5" bgColor="#EEF2FF" icon="💰" />
        <StatCard title="Total Amount" value={formatCurrency(stats.totalAmount)} color="#10B981" bgColor="#D1FAE5" icon="💵" />
        <StatCard title="Booking Payments" value={stats.bookingPayments} color="#F59E0B" bgColor="#FEF3C7" icon="📅" />
        <StatCard title="Order Payments" value={stats.orderPayments} color="#8B5CF6" bgColor="#EDE9FE" icon="📦" />
      </div>
      ...
      <ul className="nav nav-tabs mb-4">
        ...
        <li className="nav-item">
          <button className={`nav-link ${activeTab === "bookings" ? "active fw-bold" : ""}`} onClick={() => setActiveTab("bookings")}>
            🎰 Booking Payments
          </button>
        </li>
        <li className="nav-item">
      ...
      <div className="row mb-4">
        <div className="col-md-6">
          <input
            ...
            placeholder="🔍 Search by customer name, email, or transaction code..."
            value={searchTerm}
              ...
              <tbody>
                {currentPayments.map((payment, index) => (
                  <tr key={payment.id}>
                    ...
                    <td>
                      <span className={`badge ${payment.source === "booking" ? "bg-success" : "bg-info"}`}>
                        {payment.source === "booking" ? "🎰 Booking" : "📦 Order"}
                      </span>
                    </td>
                    <td>
                    ...
                    <td>{payment.method}</td>
                    <td><code className="small">{payment.transactionCode || payment.mpesaReceiptNumber || "N/A"}</code></td>
                    <td>{formatDate(payment.date)}</td>
                    <td>
                      <span className={`badge ${payment.status === "Paid" || payment.status === "Approved" ? "bg-success" : "bg-warning"}`}>
                        {payment.status}
                      </span>
                    </td>
                    <td>
                      <button
                        ...
                        data-bs-target={`#paymentModal${payment.id.replace(/[^a-zA-Z0-9]/g, '')}`}
                      >
                        👁️ View
                      </button>
                    </td>
              ...
              <div className="modal-content">
                <div className="modal-header" style={{ backgroundColor: payment.source === "booking" ? "#10B981" : "#8B5CF6", color: "white" }}>
                  <h5 className="modal-title">
                    {payment.source === "booking" ? "🎰 Booking Payment Details" : "📦 Order Payment Details"}
                  </h5>
                ...
                <div className="modal-body">
                  <div className="row">
                    <div className="col-md-6">
                      ...
                      <p className="text-muted">{payment.customerName}</p>
                    </div>
                    <div className="col-md-6">
                      <strong>Email:</strong>
                      <p className="text-muted">{payment.email}</p>
                    </div>
                  </div>
                  <div className="row">
                    <div className="col-md-6">
                      <strong>Phone:</strong>
                      <p className="text-muted">{payment.phone}</p>
                    </div>
                    <div className="col-md-6">
                  ...
                  <div className="row">
                    <div className="col-md-6">
                      ...
                      <p className="text-success fw-bold">{formatCurrency(payment.amount)}</p>
                    </div>
                    <div className="col-md-6">
                      <strong>Status:</strong>
                      <p><span className={`badge ${payment.status === "Paid" || payment.status === "Approved" ? "bg-success" : "bg-warning"}`}>{payment.status}</span></p>
                    </div>
                  </div>
                  <div className="row">
                    <div className="col-md-6">
                      <strong>Transaction Code:</strong>
                      <p><code>{payment.transactionCode || payment.mpesaReceiptNumber || "N/A"}</code></p>
                    </div>
                    <div className="col-md-6">
                      <strong>Date:</strong>
                    ...
                    <>
                      ...
                      <div className="row">
                        <div className="col-md-6">
                          ...
                          <p className="text-muted">{payment.gameTitle}</p>
                        </div>
                        <div className="col-md-6">
                          <strong>Event Date:</strong>
                          <p className="text-muted">{formatDate(payment.eventDate)}</p>
                        </div>
                      </div>
                      <div className="row">
                        <div className="col-md-6">
                          <strong>Location:</strong>
                          <p className="text-muted">{payment.location}</p>
                        </div>
                        <div className="col-md-6">
                          <strong>Paid Amount:</strong>
                          <p className="text-success fw-bold">{formatCurrency(payment.paidAmount)}</p>
                        </div>
                      </div>
                    </>
                  )}
                    ...
                    <>
                      ...
                      <div className="row">
                        <div className="col-md-6">
                          ...
                          <p className="text-muted">{payment.orderNumber}</p>
                        </div>
                        <div className="col-md-6">
                          <strong>Order Type:</strong>
                          <p className="text-muted">{payment.orderType}</p>
                        </div>
                      </div>
